...
import 'package:studentcard/services/face_database.dart';
...
class Recognizer {
  ...
  @override
  String get modelName => 'assets/model/facenet.tflite';
  ...
  void loadRegisteredFaces() async {
    registered.clear();
    final List<ResultRow> rows = await dbHelper.queryAllRow();
   ...
   //  }
    for (ResultRow row in rows) {
      String name = row['uid'];
      String decodedString = row['embedding'].toString();
      List<double> embd = decodedString.split(',').map((e) => double.parse(e)).toList().cast<double>();
      Recognition recognition = Recognition(row['uid'],Rect.zero,embd,0);
      registered.putIfAbsent(name, () => recognition);
  ...
  void registerFaceInDB(String name, List<double> embedding) async {
    await dbHelper.insertData(name, embedding.join(","));
